    ...
    def insertDB (self, curDB):
        ...
        for geneName in self._geneNames:
            ...
            if GeneName.DEBUG_INSERT_DB:
                ...
                try:
                    ## La requête 
                    curDB.prepare ("INSERT INTO ENTRY_2_GENE_NAME (accession, gene_name_id)" \
                                        + " VALUES(:accession,:gene_name_id) " )
                    ## on éxecute la requête 
                    curDB.execute (None, {'accession': self._accession, 'gene_name_id': geneNameId})
                except:
                    print("Erreur dans Entry 1..\n")
                #### FIN TODO ####
            ...
            if Keyword.DEBUG_INSERT_DB:
                ...
                try:
                    curDB.prepare ("SELECT kw_id " \
                                + " FROM keywords " \
                                + " WHERE kw_label=:kwLabel")
                    curDB.execute (None, {'kwLabel': kw._kwLabel })
                    raw = curDB.fetchone ()
                    ## La requête 
                    curDB.prepare ("INSERT INTO ENTRIES_2_KEYWORDS (accession, kw_id)" \
                                        + " VALUES(:accession,:kw_id) " )
                    ## on éxecute la requête 
                    curDB.execute (None, {'accession': self._accession, 'kw_id': raw[0]})
                except:
                    print("Erreur dans Entry 2..\n")
                #### FIN TODO ####
